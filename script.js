...
const translations = {
    ...
    pt: {
        ...
        pricingDescription: "Comece com um teste grátis de 7 dias, depois escolha o plano que atende aos seus objetivos",
        freeTrial: "Teste Grátis",
        freeTrialPlan: "Teste Grátis de 7 Dias",
        freeTrialSubtitle: "7 dias grátis • Nenhum cartão de crédito necessário",
...
function toggleLanguage() {
    ...
    const newLang = currentLang === 'en' ? 'es' : (currentLang === 'es' ? 'pt' : 'en');
...
function updateLanguageFlags() {
    ...
    flags.forEach(flag => {
        flag.textContent = langFlags[currentLang] || langFlags.en;
    });
    
    texts.forEach(text => {
        text.textContent = langTexts[currentLang] || langTexts.en;
    });
}
...
function updatePayPalForm(form, planKey) {
    ...
    const inputs = [
        ...
        { name: 'currency_code', value: PRICING_CONFIG.currency }
    ];
